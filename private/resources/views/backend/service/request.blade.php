				...
				<div class="box box-solid">
					...
					<div class="box-body">
						<div class="form-group">
							<!-- <a href="{{ url('artmin/catalog/add-product') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New Product</a> -->
							<div class="row">
								<div class="col-md-6">
									<!-- <a href="{{ url('artmin/service/request/add-service-request') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Create Service Request</a> -->
									<!-- <a href="{{ url('artmin/service/request/browse-warranty') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Create Service Request</a> -->
								</div>
								<div class="col-md-6 pull-right">
									<!-- <a href="{{ url('artmin/service/request/export-service-request-pdf') }}" class="btn btn-primary pull-right"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Export Data PDF</a> -->
						...
						<div class="col-sm-12 table-responsive">
							<table class="table data-table table-sm table-bordered table-hover service-table">
								<thead>
									<tr>
										<th><label>#</label></th>
										<th><label style="width: 95px;">Service No.</label></th>
										<th><label style="width: 95px;">Request Date</label></th>
										<th><label style="width: 100px;">Prefered Visit</label></th>
										<th><label style="width: 130px;">Name</label></th>
										<th><label style="width: 80px;">Phone</label></th>
								...
								<tbody>
									...
									<?php foreach ($service as $srv) : ?>
										<tr>
											<td><?= $i++ ?></td>
											<td><span style="width: 95px;">{{ $srv->service_no }}</span></td>
											<td><span style="width: 95px;">{{ date('d-m-Y', strtotime($srv->created_at)) }}</span></td>
											<td><span style="width: 100px;">{{ date('d-m-Y', strtotime($srv->prefered_date)) }}</span></td>
											<td><span style="width: 130px;">{{ $srv->contact_name }}</span></td>
											<td><span style="width: 80px;">{{ $srv->contact_phone }}</span></td>
											...
											<td>
												<center>
												<a href="{{ url('artmin/service/request-details/' . $srv->service_id) }}" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>
...
<script>
	...
	$(document).on('click', '.btn-submit-delete', function(e) {
		...
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					let url = '{{ url("artmin/service/delete") }}';
					let data = {
						"_token": "{{ csrf_token() }}",
	...
	$(document).on('click', '.btn-exec-export', function(e) {
		...
		let url = `{{ url('artmin/service/request/export-service-request-${rptType}') }}` + '/' + tglFrom + '/' + tglTo;
	...
	$(document).ready( function() {
		...
		$('.service-table').DataTable({
			...
			ajax: '{!! route('artmin.service.list') !!}', // memanggil route yang menampilkan data json
			columns: [
				{
					data: "service_type",
					name: "service_type",
				...
				},
				{
					...
					searchable: false,
					render : function (data, type, row) {
						// console.log("ZZZZZ--data", data);
						// console.log("ZZZZZ--type", type);
						// console.log("ZZZZZ--row", row);
						if(data === 0){
							return 'On Progress';
						} else if(data === 1) {
							return 'Completed';
				...
				},
				{
					...
					render : function (data, type, row) {
						...
						return `
							<center>
							<a href="{{ url('artmin/service/request-details/${row.service_id}') }}" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>
							${delBtn}
							</center>
						`;
					}
