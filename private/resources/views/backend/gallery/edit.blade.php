    ...
    <section class="content">
        ...
        <div class="row">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="box box-solid">
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <label>Title</label>
                                <input type="text" class="form-control" name="title" id="title" value="{{ old('title', $gallery->title) }}">
                                @if ($errors->has('title'))
                            ...
                            <div class="form-group">
                                <label>Slug</label>
                                <input type="text" class="form-control" name="slug" id="url" value="{{ old('slug', $gallery->slug) }}">
                                @if ($errors->has('slug'))
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" name="image" id="image" value="<?= old('image', $gallery->image) ?>"/>
                                @if ($errors->has('image'))
                                <label class="control-label input-error" for="inputError"><i class="fa fa-exclamation-circle"></i> {{ $errors->first('image') }}</label>
                                @endif
                            </div>
                            <div class="form-group">
                                <img id="preview_image" class="preview" src="<?= old('image', $gallery->image) ?>"/>
                            </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="box box-solid">
                        <div class="box-body">
                            ...
                            <input type="hidden" id="image_count" name="image_count" value="<?= count($gallery_images) ?>">
                            <div id="gallery-image-container">
                                <?php $i = 1; ?>
                                <?php foreach ($gallery_images as $img): ?>
                                    <div class="gallery-images" id="gallery-image-<?= $i ?>">
                                        <a href="<?= url('fmanager/dialog.php?field_id=image-<?= $i ?>&akey=R4z5OEYf1h29rzVepedx') ?>" class="iframe-btn btn btn-xs bg-black"><i class="fa fa-search"></i> Browse</a><br /><a href="javascript:void(0)" class="remove-gallery" onclick="remove_images(<?= $i ?>)"><i class="fa fa-times"></i></a>
                                        <input type="hidden" name="gallery_image[]" id="image-<?= $i ?>" value="<?= $img->images ?>"/>
                                        <div class="form-group">
                                            <img id="preview_image-<?= $i ?>" class="preview" src="<?= $img->images ?>"/>
                                        </div>
                                    </div>
                                    <?php $i++; ?>
                                <?php endforeach; ?>
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        <button class="btn btn-primary"><i class="fa fa-save"></i> Update Gallery</button>
                    </div>
                </div>
            </form>
    </section><!-- /.content -->
